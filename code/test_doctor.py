...
for i in range(3):
    ...
    imgs_a, preds_a = np.loadtxt(data_dir + doctors[2 * i], dtype=str, usecols=[1, 2]).T
    imgs_b, preds_b = np.loadtxt(data_dir + doctors[2 * i + 1], dtype=str, usecols=[1, 2]).T
    ...
    for cross_val_index in cross_val_lists:
        ...
        TRAIN_FILE = "D:/LDL/Acne04/Detection/VOC2007/ImageSets/Main/NNEW_trainval_" + cross_val_index + '.txt'
        TEST_FILE = 'D:/LDL/Acne04/Detection/VOC2007/ImageSets/Main/NNEW_test_' + cross_val_index + '.txt'

        imgs_train, labels_train = np.loadtxt(TRAIN_FILE, dtype=str, usecols=[0, 1]).T
        labels_train = labels_train.astype(int)

        imgs_test, labels_test = np.loadtxt(TEST_FILE, dtype=str, usecols=[0, 1]).T
